...
class ModdedDex {
	...
	validateBanRule(rule) {
		...
		for (const matchType of matchTypes) {
			...
			case 'pokemontag':
				...
				const validTags = [
					// singles tiers
					'uber', 'ou', 'uubl', 'uu', 'rubl', 'ru', 'nubl', 'nu', 'publ', 'pu', 'zubl', 'zu', 'nfe', 'lcuber', 'lc', 'cap', 'caplc', 'capnfe',
					//doubles tiers
					'duber', 'dou', 'dbl', 'duu',
					// custom tags
					'mega',
	...
	escapeHTML (str) {
		process.emitWarning('Dex.prototype.escapeHTML is deprecated.', 'DeprecationWarning');
		if (!str) return '';
		return ('' + str).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;').replace(/'/g, '&apos;').replace(/\//g, '&#x2f;');
	}

	html (strings) {
		process.emitWarning('Dex.prototype.html is deprecated.', 'DeprecationWarning');
		let buf = strings[0];
		for (let i = 1; i < arguments.length; i++) {
			buf += moddedDex.base.escapeHTML(arguments[i]);
			buf += strings[i];
		}
		return buf;
	}
	plural (num, plural, singular) {
		process.emitWarning('Dex.prototype.plural is deprecated.', 'DeprecationWarning');
		if (!plural) plural = 's';
	...
	toTimeStamp (date, options) {
		process.emitWarning('Dex.prototype.toTimeStamp is deprecated.', 'DeprecationWarning');
	...
	toDurationString (number, options) {
		process.emitWarning('Dex.prototype.toDurationString is deprecated.', 'DeprecationWarning');
